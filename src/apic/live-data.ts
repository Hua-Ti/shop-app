...
export async function getLiveGoods(roomId: number, actorId: string) {
    let { data } = await axios.get<liveRoomGood>(`https://apic.netstart.cn/mogujie/live/queryDiscountPrice?roomId=${roomId}&actorId=${actorId}`)
    return data
}
//直播间评论
export async function getComment() {
    let { data } = await axios.get<liveComment>(`https://apic.netstart.cn/mogujie/live/comments`)
    return data
}

// 直播回放相关
export async function getPlaybackData(itemUrlId: string, actorUrlId: string) {
    let { data } = await axios.get<getPlaybackData>(`https://apic.netstart.cn/mogujie/video/detail?itemId=${itemUrlId}&actorId=${actorUrlId}`)
    return data
}
//商品数据
export async function getGood(itemId:string){
    let { data } = await axios.get<LiveDetailList>(`https://apic.netstart.cn/mogujie/goods/detail?itemId=${itemId}`)
    return data
}
